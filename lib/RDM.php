...
class RDM extends Scanner
{
    public function get_active($eids, $minIv, $minLevel, $exMinIv, $bigKarp, $tinyRat, $swLat, $swLng, $neLat, $neLng, $tstamp = 0, $oSwLat = 0, $oSwLng = 0, $oNeLat = 0, $oNeLng = 0, $encId = 0)
    {
        ...
        $select = "pokemon_id, expire_timestamp AS disappear_time, spawn_id AS encounter_id, lat AS latitude, lon AS longitude, gender, form, weight, weather AS weather_boosted_condition";
        ...
        $encSql = '';
        if ($encId != 0) {
            $encSql = " OR (spawn_id = " . $encId . " AND lat > '" . $swLat . "' AND lon > '" . $swLng . "' AND lat < '" . $neLat . "' AND lon < '" . $neLng . "' AND expire_timestamp > '" . $params[':time'] . "')";
        }
