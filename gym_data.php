...
global $map;
if ($map == "monocle") {
    $row = $db->query("select distinct t3.external_id, t3.lat, t3.lon, t1.last_modified, t1.team, t1.slots_available, t1.guard_pokemon_id, t4.raid_level as level, t4.pokemon_id, t4.cp, t4.move_1, t4.move_2, t4.raid_start as battle, t4.raid_end as end from(select fort_id, MAX(last_modified) AS MaxLastModified from fort_sightings group by fort_id) t2 left join fort_sightings t1 on t2.fort_id = t1.fort_id and t2.MaxLastModified = t1.last_modified left join forts t3 on t1.fort_id = t3.id join raid_info t4 on t1.fort_id = t4.fort_id where external_id = '"  . $id . "' order by external_id, raid_start desc")->fetch();
} else {
