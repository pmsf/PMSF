...
function getReward(item) {
    ...
    if (item['quest_reward_type'] === 8) {
        rewardImage = '<img style="' + styleStr + '" src="' + iconpath + 'rewards/reward_pokecoin_' + item['quest_pokecoin_amount'] + '.png"/>'
        ...
        if (!noTeamRocket && item['incident_expiration'] > Date.now()) {
            ...
            if (item['quest_reward_type'] === 8) {
                html = '<div style="position:relative;">' +
                    ...
                    '<img src="' + iconpath + 'rewards/reward_pokecoin_' + item['quest_pokecoin_amount'] + '.png" style="width:30px;height:auto;position:absolute;top:4px;left:0px;"/>' +
                    '</div>'
                stopMarker = L.divIcon({
                    iconSize: [31, 31],
                    iconAnchor: [25, 45],
